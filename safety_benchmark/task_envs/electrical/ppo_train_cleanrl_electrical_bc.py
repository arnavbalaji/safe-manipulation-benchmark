...
class Args:
    ...
    bc_coef: float = 0.1
    """maximum coefficient for auxiliary behavior cloning loss from top-K replay buffer (linearly annealed from 0 to this value over training)"""
    bc_buffer_size: int = 10
    """maximum number of episodes to store in the BC replay buffer"""
    bc_batch_size: int = 128
    """number of transitions to sample from BC buffer per PPO minibatch"""
...
if __name__ == "__main__":
    ...
    # Self-imitation / BC replay buffer: store top-K full episodes by return
    bc_buffer = []
    episode_obs_buffer = [[] for _ in range(args.num_envs)]
    episode_action_buffer = [[] for _ in range(args.num_envs)]
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # Linearly increase BC coefficient from 0 to args.bc_coef over total timesteps
        if args.bc_coef > 0.0 and args.total_timesteps > 0:
            bc_frac = min(1.0, float(global_step) / float(args.total_timesteps))
            current_bc_coef = args.bc_coef * bc_frac
        else:
            current_bc_coef = 0.0
        ...
        for step in range(0, args.num_steps):
            ...
            # Track per-episode (obs, action) pairs for BC buffer (use pre-step obs and chosen action)
            with torch.no_grad():
                for env_i in range(args.num_envs):
                    episode_obs_buffer[env_i].append(next_obs[env_i].detach().cpu().numpy())
            # ALGO LOGIC: action logic
            with torch.no_grad():
                action, logprob, _, value = agent.get_action_and_value(next_obs)
                values[step] = value.flatten()
                # Cache actions for BC buffer alongside the obs we just stored
                for env_i in range(args.num_envs):
                    episode_action_buffer[env_i].append(action[env_i].detach().cpu().numpy())
            actions[step] = action
            ...
            if "final_info" in infos:
                for env_i, info in enumerate(infos["final_info"]):
                    if info and "episode" in info:
                        ...
                        # Build episode trajectory for BC buffer (truncate to reported length)
                        ep_obs = np.asarray(episode_obs_buffer[env_i], dtype=np.float32)
                        ep_actions = np.asarray(episode_action_buffer[env_i], dtype=np.float32)
                        if ep_obs.shape[0] > ep_len:
                            ep_obs = ep_obs[:ep_len]
                        if ep_actions.shape[0] > ep_len:
                            ep_actions = ep_actions[:ep_len]
                        if ep_obs.shape[0] > 0 and ep_obs.shape[0] == ep_actions.shape[0]:
                            ep_entry = {
                                "return": ep_return,
                                "length": int(ep_obs.shape[0]),
                                "obs": ep_obs,
                                "actions": ep_actions,
                            }
                            # Insert into top-K buffer
                            if len(bc_buffer) < args.bc_buffer_size:
                                bc_buffer.append(ep_entry)
                            else:
                                returns = [e["return"] for e in bc_buffer]
                                worst_idx = int(np.argmin(returns))
                                if ep_return > returns[worst_idx]:
                                    bc_buffer[worst_idx] = ep_entry
                        # Clear per-episode buffers for this env
                        episode_obs_buffer[env_i] = []
                        episode_action_buffer[env_i] = []
        ...
        # Flatten BC buffer once per iteration for efficiency (if enabled)
        bc_obs_flat = None
        bc_actions_flat = None
        bc_total = 0
        if current_bc_coef > 0.0 and len(bc_buffer) > 0:
            bc_obs_list = [torch.tensor(ep["obs"], dtype=torch.float32, device=device) for ep in bc_buffer]
            bc_actions_list = [torch.tensor(ep["actions"], dtype=torch.float32, device=device) for ep in bc_buffer]
            bc_obs_flat = torch.cat(bc_obs_list, dim=0)
            bc_actions_flat = torch.cat(bc_actions_list, dim=0)
            bc_total = int(bc_obs_flat.shape[0])
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                # Auxiliary behavior cloning loss from top-K replay buffer
                bc_loss = torch.tensor(0.0, device=device)
                if current_bc_coef > 0.0 and bc_total > 0:
                    bc_bs = min(args.bc_batch_size, bc_total)
                    bc_indices = np.random.randint(0, bc_total, size=bc_bs)
                    bc_obs_batch = bc_obs_flat[bc_indices]
                    bc_actions_batch = bc_actions_flat[bc_indices]
                    # get_action_and_value returns logprob for provided actions (squashed in [-1,1])
                    _, bc_logprob, _, _ = agent.get_action_and_value(bc_obs_batch, bc_actions_batch)
                    bc_loss = -bc_logprob.mean()
                    bc_losses.append(float(bc_loss.detach().cpu().item()))

                entropy_loss = entropy.mean()
                loss = pg_loss - current_ent_coef * entropy_loss + v_loss * args.vf_coef + current_bc_coef * bc_loss
        ...
        # Aggregate BC metrics
        mean_bc_loss = float(np.mean(bc_losses)) if len(bc_losses) > 0 else 0.0
        bc_buffer_avg_return = float(np.mean([e["return"] for e in bc_buffer])) if len(bc_buffer) > 0 else 0.0
        ...
        wandb.log({
            ...
            "charts/bc_coef": float(current_bc_coef),
            ...
            "losses/bc_loss": mean_bc_loss,
            "charts/bc_buffer_avg_return": bc_buffer_avg_return,
