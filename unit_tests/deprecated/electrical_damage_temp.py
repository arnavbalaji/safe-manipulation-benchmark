"""
Electrical damage temp scene (faucet + counter):
- Same faucet / sink setup as electrical_damage.py
- Target object (laptop) is on the counter, not inside the sink
- Adds a glass on the counter (not filled)
- Teleoperation enabled, recordings for laptop health and water contacts
"""
...
TARGET_OBJECT_CONFIGS = {
    # Laptop used in this temp script (position changed to counter)
    "laptop": {
        "type": "DatasetObject",
        "name": "target_object",
        "category": "laptop",
        "model": "nvulcs",
        # CHANGED: place on the counter instead of near sink basin
        "position": [-0.15, 0.27, 0.8],
        "orientation": [0.0, 0.0, 0.0, 1.0],
        "scale": [1.0, 1.0, 1.0],
        "damage_params": PARAMS["laptop"],
    },
    # Toy car from BEHAVIOR dataset (unchanged; not used here but kept for parity)
    "toy_car": {
        "type": "DatasetObject",
        "name": "target_object",
        "category": "toy_car",
...
def create_scene_config(target_object_key="laptop"):
    """Create the scene configuration with robot, sink, target object, and an added glass."""
    ...
    # ADDED: yellow mug on the counter (not filled)
    mug_obj = {
        "type": "DatasetObject",
        "name": "mug_ppzttc",
        "category": "mug",
        "model": "ppzttc",
        # Counter position taken from task1.yaml box_of_crackers
        "position": [-0.4, 0.25, 0.8],
        "orientation": [0, 0, 0, 1],
        "scale": [1.0, 1.0, 1.0],
    }
    objects.append(mug_obj)
...
def main():
    ...
    # CHANGED: Use laptop as target (instead of toy_car)
    target_object_key = "laptop"
