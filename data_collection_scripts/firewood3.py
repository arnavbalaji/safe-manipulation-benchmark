...
def _ensure_firewood_states(env):
    ...
    This ensures:
    - Robot has Temperature state (for thermal health tracking) and is registered in Temperature.OBJ_IDXS
    ...
    if env.robots:
        ...
        # CRITICAL: Ensure robot has Temperature state and it's initialized
        # This is required for HeatSourceOrSink to detect the robot via Temperature.OBJ_IDXS
        if object_states.Temperature not in robot.states:
            ...
            # Ensure existing Temperature state is initialized
            temp_state = robot.states[object_states.Temperature]
            if not temp_state._initialized:
                temp_state.initialize()
    ...
    if fireplace is not None:
        ...
        # Always ensure HeatSourceOrSink state exists and is properly configured
        if object_states.HeatSourceOrSink not in fireplace.states:
            ...
            # Update existing state parameters if they've changed
            heat_source_state = fireplace.states[object_states.HeatSourceOrSink]
            # Update parameters (they're stored as instance variables)
            heat_source_state._temperature = heat_source_cfg.get("temperature", 100.0)
            heat_source_state._heating_rate = heat_source_cfg.get("heating_rate", 0.1)
            heat_source_state.distance_threshold = heat_source_cfg.get("distance_threshold", 0.15)
    ...
    # Step a few times to let states settle and update
    for _ in range(5):
        og.sim.step()
...
def _debug_heat_source_distance(env, robot):
    """
    Enhanced debug function showing heat source details: distance, threshold, heating_rate, and positions.
    """
    from omnigibson.object_states.temperature import Temperature
    
    # Check fireplace HeatSourceOrSink
    fireplace = env.scene.object_registry("name", "fireplace")
    fireplace_affected = []
    fireplace_active = False
    heat_source_state = None
    heat_source_pos = None
    distance_threshold = None
    heating_rate = None
    robot_distance = None
    robot_in_temp_idx = False
    
    if fireplace is not None and object_states.HeatSourceOrSink in fireplace.states:
        heat_source_state = fireplace.states[object_states.HeatSourceOrSink]
        fireplace_active = heat_source_state.get_value()
        
        # Get heat source parameters
        distance_threshold = heat_source_state.distance_threshold
        heating_rate = heat_source_state.heating_rate
        
        # Get heat source position (same logic as HeatSourceOrSink._update)
        # Check if links exist before accessing link property (which asserts)
        if hasattr(heat_source_state, 'links') and heat_source_state.links:
            try:
                link = heat_source_state.link
                if link == heat_source_state._default_link:
                    heat_source_pos = link.aabb_center
                else:
                    heat_source_pos = link.get_position_orientation()[0]
            except (AssertionError, AttributeError):
                # Fallback to fireplace position if link access fails
                heat_source_pos = fireplace.get_position_orientation()[0]
                if isinstance(heat_source_pos, th.Tensor):
                    heat_source_pos = heat_source_pos.clone()
        else:
            # Fallback to fireplace position
            heat_source_pos = fireplace.get_position_orientation()[0]
            if isinstance(heat_source_pos, th.Tensor):
                heat_source_pos = heat_source_pos.clone()
        
        if heat_source_state._affected_objects:
            fireplace_affected = [obj.name for obj in heat_source_state._affected_objects]
        
        # Calculate robot distance to heat source
        try:
            # Get robot EEF position (most relevant for manipulation)
            if hasattr(robot, 'eef_links') and robot.eef_links:
                robot_pos = th.tensor(robot.eef_links[0].get_position_orientation()[0])
            else:
                robot_pos = th.tensor(robot.get_position_orientation()[0])
            
            robot_distance = th.norm(heat_source_pos - robot_pos).item()
        except Exception as e:
            robot_distance = f"Error: {e}"
        
        # Check if robot is in Temperature.OBJ_IDXS (required for HeatSourceOrSink to affect it)
        if hasattr(Temperature, 'OBJ_IDXS'):
            robot_in_temp_idx = robot in Temperature.OBJ_IDXS
    
    # Check OnFire states on all objects
    onfire_affected = {}
    onfire_objs = []
    for obj in env.scene.objects:
        if object_states.OnFire in obj.states:
            onfire_state = obj.states[object_states.OnFire]
            if onfire_state.get_value():
                onfire_objs.append(obj.name)
                if onfire_state._affected_objects:
                    onfire_affected[obj.name] = [o.name for o in onfire_state._affected_objects]
    
    # Check if robot is affected by any heat source
    robot_affected_by_fireplace = robot.name in fireplace_affected if fireplace_affected else False
    robot_affected_by_onfire = []
    for obj_name, affected_list in onfire_affected.items():
        if robot.name in affected_list:
            robot_affected_by_onfire.append(obj_name)
    
    # Print debug info (only print every 10 steps to reduce spam)
    if not hasattr(_debug_heat_source_distance, 'step_count'):
        _debug_heat_source_distance.step_count = 0
    
    _debug_heat_source_distance.step_count += 1
    should_print = _debug_heat_source_distance.step_count % 10 == 0
    
    # Get robot temperature
    robot_temp = None
    if object_states.Temperature in robot.states:
        robot_temp = robot.states[object_states.Temperature].get_value()
        if isinstance(robot_temp, th.Tensor):
            robot_temp = robot_temp.item() if robot_temp.numel() == 1 else robot_temp.tolist()
    
    if should_print:
        # Build one-line output
        parts = [f"Step {_debug_heat_source_distance.step_count}"]
        
        if heat_source_state is not None:
            parts.append(f"Fireplace: active={fireplace_active}")
            parts.append(f"threshold={distance_threshold}")
            parts.append(f"rate={heating_rate}")
            if robot_distance is not None and isinstance(robot_distance, (int, float)):
                within_threshold = robot_distance <= distance_threshold
                parts.append(f"dist={robot_distance:.3f}")
                parts.append(f"within={within_threshold}")
            parts.append(f"robot_in_temp_idx={robot_in_temp_idx}")
            parts.append(f"affected={fireplace_affected}")
        else:
            parts.append("Fireplace: NOT FOUND")
        
        if robot_temp is not None:
            if isinstance(robot_temp, (list, tuple)):
                parts.append(f"robot_temp={[f'{t:.1f}' for t in robot_temp]}")
            else:
                parts.append(f"robot_temp={robot_temp:.1f}")
        
        if onfire_objs:
            parts.append(f"OnFire={onfire_objs}")
            for obj_name, affected in onfire_affected.items():
                parts.append(f"{obj_name}_affected={affected}")
        
        parts.append(f"robot_affected_by_fireplace={robot_affected_by_fireplace}")
        parts.append(f"robot_affected_by_onfire={robot_affected_by_onfire}")
        
        print(f"[HEAT_DEBUG] {' | '.join(parts)}")
        ...
        while completed_episodes < n_episodes:
            ...
            while True:
                ...
                if episode_starts:
                    ...
                    # Debug heat source distance after step
                    _debug_heat_source_distance(env, robot)
                else:
